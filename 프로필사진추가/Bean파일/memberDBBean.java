...
public class memberDBBean {
	...
	public int insertMember(memberBean member) throws Exception{		//회원가입을 위한 메소드
		...
		String sql = "insert into users_user(email, password, username, birthday, userphoto)"
				+ " values(?,?,?,?,?)";
		int isValid = 0;
		...
		try {
			...
			pstmt.setString(5, member.getUserphoto());  //프로필 사진 추가
	...
	public memberBean getMember(String email) throws Exception {	//회원 정보 
		...
		try {
			...
			if(rs.next()) {
				...
				member.setUserphoto(rs.getString("userphoto"));		//프로필 사진 추가
	...
	public int updateMember(memberBean member) throws Exception {	//회원 비밀번호 수정
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = "update users_user set password = ?, userphoto = ? where email = ?";
		int re = -1;	//기본 설정 값 -1
